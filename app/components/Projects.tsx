...
export default function Projects() {
  ...
  const x: any = useTransform(scrollYProgress, [0, 1], ["1%", "-100%"]);
  const images: StaticImageData[] = [card4, card6, card3, card5, card1, card2];
  return (
    <div
      ref={targetContainer}
      className=" text-white  h-[300vh] bg-[var(--backgroundBlack)]"
    >
      <div className="sticky top-0 p-20 left-0 w-full flex   overflow-hidden h-screen  justify-center flex-col items-start">
        <h1 className="text-7xl p-20">Projects</h1>
        <motion.div style={{ x }} className="flex  w-full  gap-10">
          {images.map((item: StaticImageData, index: number) => (
            <Card title="title" imageData={item} key={index} />
          ))}
        </motion.div>
      </div>
    </div>
